...
EPCOHS = 400 #  an arbitrary cutoff, generally defined as "one pass over the entire dataset", used to separate training into distinct phases, which is useful for logging and periodic evaluation.
BATCH_SIZE = 81 # a set of N samples. The samples in a batch are processed` independently, in parallel. If training, a batch results in only one update to the model.
INPUT_DIM = 4 # a vocabulary of 4 words in case of fnn sequence (ATCG)
OUTPUT_DIM = 1 # Embedding output
RNN_HIDDEN_DIM = 64
DROPOUT_RATIO = 0.4 # proportion of neurones not used for training
MAXLEN = 14 # cuts text after number of these characters in pad_sequences
